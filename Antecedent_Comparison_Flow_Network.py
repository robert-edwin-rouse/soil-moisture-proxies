...
targets = ['Flow']
xspace = hu.featurelocator(rf, features)
yspace = hu.featurelocator(rf, targets)
...
norm_cache = {}
for f in features:
    rftrain[f] = hu.normalise(rftrain, f, norm_cache, write_cache=True)
    rf[f] = hu.normalise(rf, f, norm_cache, write_cache=False)
...
for df in (testrf, rf):
    ...
    print('- - - - - - - - - - - - - - -')
    print('RMSE: ' + str((slm.mean_squared_error(df['Flow'], df['Predicted']))**(1/2)))
    print('MAE: '  + str(slm.mean_absolute_error(df['Flow'], df['Predicted'])))
    print('R\N{SUPERSCRIPT TWO}: ' + str(slm.r2_score(df['Flow'], df['Predicted'])))
hu.year_plot(maxflow, rf, 'Predicted', 'Flow', 2007)
hu.year_plot(maxflow, rf, 'Predicted', 'Flow', 2012)
